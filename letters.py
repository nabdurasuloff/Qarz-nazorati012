...
def birlashtir_docx(fayllar_royxati, output_path):
    """
    Bir nechta .docx faylni (masalan tanlangan mijozlarning Davo arizalari)
    BITTA umumiy Word faylga jamlaydi — har biri orasida yangi sahifadan
    boshlanadi. docxcompose kutubxonasi asosida ishlaydi.
    ...
    composer = Composer(asosiy)
    for fayl in fayllar_royxati[1:]:
        asosiy.add_page_break()
        qoshimcha = Document(fayl)
        composer.append(qoshimcha)
